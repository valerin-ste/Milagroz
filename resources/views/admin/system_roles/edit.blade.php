...
<div class="d-flex justify-content-between align-items-center mt-3 mb-2 px-2">
    <div>
        <h2 class="fw-bold mb-1" style="color: var(--text-main); font-size: 1.75rem; letter-spacing: -0.5px;">
            Editar Rol de Sistema
        </h2>
        <p class="text-muted mb-0" style="font-size: 0.95rem;">Rol: <strong>{{ $role->name }}</strong></p>
    </div>
    <a href="{{ route('admin.system_roles.index') }}" class="btn btn-light-custom px-4">
                ...
                <div class="card-body px-4 pb-4 pt-2">
                    <form action="{{ route('admin.system_roles.update', $role) }}" method="POST">
                            ...
                            <div class="col-md-12">
                                ...
                                @if($role->name === 'Admin')
                                    <small class="text-warning d-block mt-1">
                                        <i class="fas fa-lock me-1"></i> El rol Administrador principal no puede cambiar de nombre.
                                    </small>
                                @endif
                            </div>

                            <div class="col-12">
                                <label class="form-label fw-bold">Asignación de Permisos</label>
                                <p class="text-muted small mb-3">Seleccione a qué módulos y acciones tendrá acceso este rol.</p>

                                <div class="row g-2">
                                    @forelse($permissions as $permission)
                                        @php
                                            $checked = (is_array(old('permissions')) && in_array($permission->name, old('permissions')))
                                                    || (isset($role) && $role->hasPermissionTo($permission->name));
                                            $disabled = $role->name === 'Admin';
                                        @endphp
                                        <div class="col-md-4">
                                            <div class="role-check-card {{ ($checked || $disabled) ? 'selected' : '' }}">
                                                ...
                                                <label for="perm_{{ $permission->id }}" class="form-check-label ms-2">
                                                    {{ $permission->name }}
                                                </label>
                                            </div>
                                        </div>
                                    @empty
                                        <div class="col-12">
                                            <div class="alert" style="background-color: #eff6ff; color: #1d4ed8; border: none; border-radius: 8px;">
                                                <i class="fas fa-info-circle me-2"></i> No hay permisos definidos en el sistema todavía.
                                            </div>
                                        </div>
                                    @endforelse
                                </div>
                        ...
                        <div class="d-flex justify-content-end gap-3 mt-5 border-top pt-4">
                            <a href="{{ route('admin.system_roles.index') }}" class="btn btn-light-custom px-4">Cancelar</a>
