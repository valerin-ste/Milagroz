...
<div class="page-header d-flex justify-content-between align-items-center mb-2 px-2">
    <div>
        ...
        <p class="page-subtitle text-muted mb-0">
            Actualizar seguimiento o productividad.
        </p>
    </div>
        ...
        <div class="col-lg-8 mx-auto">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-0 px-4">
                    <h5 class="fw-bold" style="color: #334155;"><i class="fas fa-edit text-primary me-2"></i> Modificar Detalles</h5>
                </div>
                
                <form action="{{ route('admin.productividades.update', $productividad->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-12">
                                ...
                                <select name="empleado_id" class="form-select border-light shadow-sm bg-light select2" required>
                                    ...
                                    @foreach($empleados as $emp)
                                        <option value="{{ $emp->id }}" {{ old('empleado_id', $productividad->empleado_id) == $emp->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="titulo" class="form-control border-light shadow-sm bg-light" 
                                       placeholder="Ej: Seguimiento Mensual" value="{{ old('titulo', $productividad->titulo) }}" required maxlength="150">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="tipo" class="form-control border-light shadow-sm bg-light" 
                                       placeholder="Ej: Observación, Meta, General" value="{{ old('tipo', $productividad->tipo) }}" maxlength="100">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="fecha" class="form-control border-light shadow-sm bg-light" 
                                       value="{{ old('fecha', $productividad->fecha) }}" required>
                            </div>

                            {{-- ARCHIVO --}}
                            <div class="col-md-6">
                                <label class="form-label fw-bold text-muted small">Reemplazar Archivo <span class="text-muted">(Opcional)</span></label>
                                <input type="file" name="archivo" class="form-control border-light shadow-sm bg-light">
                                <small class="text-muted mt-1 d-block">Tamaño máximo 5MB. Deja vacío para mantener el actual.</small>
                                
                                @if($productividad->archivo)
                                    <div class="mt-2 p-2 bg-white border rounded">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <span class="small text-truncate"><i class="fas fa-file-alt text-primary me-1"></i> Archivo actual</span>
                                            <a href="{{ route('admin.productividades.archivo.view', $productividad->id) }}" target="_blank" class="btn btn-sm btn-light border py-0 px-2 small">Ver</a>
                                        </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea name="descripcion" rows="4" class="form-control border-light shadow-sm bg-light" 
                                          placeholder="Describe las actividades..." required>{{ old('descripcion', $productividad->descripcion) }}</textarea>
                            </div>
